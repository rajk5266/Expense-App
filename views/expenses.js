...
window.addEventListener('DOMContentLoaded',
    async () => {
        try {
            ...
            const response = await axios.get(`http://localhost:5000/api/userData/1/${limit}`, tokentosend)
            // console.log(response.data.expenses)
            const pageButton = response.data.totalPages
            const ispremium = await axios.get('http://localhost:5000/user/profile', tokentosend)
...
async function adding(event) {
    ...
    if (updateId) {
        console.log(obj)
        try {
            const response = await axios.put(`http://localhost:5000/api/userData/${updateId}`, obj, tokentosend)
        ...
        try {
            const response = await axios.post('http://localhost:5000/api/userData', obj, tokentosend)
            console.log(response.data)
            showExpensesList(response.data);
        } catch (err) {
            console.log(err)
...
function showExpensesList(obj) {
    ...
    delBtn.onclick = async () => {
        ...
        try {
            const deleteId = obj._id;
            console.log(obj._id)
            const response = await axios.delete(`http://localhost:5000/api/userData/${deleteId}`, tokentosend);
...
async function handlePagination(page, limit) {
    try {
        const response = await axios.get(`http://localhost:5000/api/userData/${page}/${limit}`, tokentosend);
...
premiumButton.onclick = async function (e) {
    ...
    const response = await axios.get('http://localhost:5000/purchase/premiumMembership', tokentosend)
    // console.log(response)
    var options = {
        ...
        "handler": async function (response) {
            const resp = await axios.post('http://localhost:5000/purchase/premiumMembership', {
    ...
    rzp1.on('payment.failed', async function (response) {
        ...
        console.log(data)
        await axios.post('http://localhost:5000/purchase/cancelPremium', { orderid: data }, tokentosend)
